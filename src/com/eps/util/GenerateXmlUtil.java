...
import com.eps.service.DealService;
...
public class GenerateXmlUtil {
     ...
     * @return
     */
    public static Workbook exportComplexExcel(HSSFWorkbook work, HSSFSheet sheet, List<List<String>> listHeaders, List<List<Object>> datas,
                                              List<String> needMoreWidthField, String sheetName, List<CellRangeAddress> merges, int col, int startCol) {
        if (listHeaders == null && datas == null) {
            GenerateXmlUtil.addData(listHeaders, datas, null, work, sheet, null, null, null, null, col);
            return null;
        }
        ...
        for (int i = 0; i < headers2.size(); i++) {
            ...
            String field = DealService.HEADER_MAP.get(header);
            String fieldType = DealService.FIELD_TYPE_RECORD.get(field);
            ...
            colWidth = colWidth >= 5000 ? colWidth : 5000;// 最小宽度 2000
        ...
        DealService.setBorder(castTitleStyle, true, true, true, true, HSSFCellStyle.BORDER_THIN);
        ...
        DealService.setBorder(unalterTitleStyle, true, true, true, true, HSSFCellStyle.BORDER_THIN);
        ...
        DealService.setBorder(updateTitleStyle, true, true, true, true, HSSFCellStyle.BORDER_THIN);
        ...
        DealService.setBorder(contentStyle, true, true, true, true, HSSFCellStyle.BORDER_THIN);
        ...
        DealService.setBorder(passStyle, true, true, true, true, HSSFCellStyle.BORDER_THIN);
        ...
        DealService.setBorder(failStyle, true, true, true, true, HSSFCellStyle.BORDER_THIN);
        ...
        // 插入表格表头数据
        GenerateXmlUtil.addHeader(listHeaders, sheet, castTitleStyle, unalterTitleStyle, updateTitleStyle, null, startCol);
        // 插入表格内容数据
        GenerateXmlUtil.addData(listHeaders, datas, null, work, sheet, contentStyle, null, passStyle, failStyle, col);
    ...
    private static void addHeader(List<List<String>> listHeaders, HSSFSheet sheet, HSSFCellStyle castTitleStyle,
                                  HSSFCellStyle unalterTitleStyle, HSSFCellStyle updateTitleStyle, Integer[] rowHeight, int startCol) {
        for (int m = 0, n = listHeaders.size(); m < n; m++) {
            HSSFRow row = sheet.createRow((short) startCol + m);
            if (rowHeight != null && rowHeight[0] == m) {
                row.setHeightInPoints(rowHeight[1]);
            } else {
                // 设置默认行高
                sheet.setDefaultRowHeightInPoints(150);
            }
            ...
            for (int i = 0; i < headers.size(); i++) {
                ...
                if (caseHeaders.contains(header)) {
                    String titleColor = DealService.HEADER_COLOR_RECORD.get(header);
                    ...
                    if (titleColor != null && titleColor != "" && titleColor.equals("C")) {
                        cell.setCellStyle(castTitleStyle);
                    } else if ("测试用例".equals(cell.getStringCellValue())) {
                        cell.setCellStyle(castTitleStyle);
                    } else {
                        cell.setCellStyle(unalterTitleStyle);
                    }
    ...
    private static void addData(List<List<String>> listHeaders, List<List<Object>> datas, List<Integer[]> subheads,
                                ...
                                HSSFCellStyle passStyle, HSSFCellStyle failStyle, int col) {
        if (listHeaders == null && datas == null) {
            if (sheet.getRow(0) != null) {
                HSSFRow row = sheet.createRow(col);
                row.createCell(0).setCellValue("测试用例及结果");
                return;
            }
            HSSFRow row = sheet.createRow(0);
            row.createCell(0).setCellValue("测试基本信息");
            return;
        }
        ...
        for (int rowNo = 0, max = datas.size(); rowNo < max; rowNo++) {
            ...
            int rowNum = col + headerSize;
            HSSFRow row = sheet.createRow((short) rowNum + rowNo);// 计算出romNum
            ...
            for (int i = 0, j = data.size(); i < j; i++) {
                ...
                String headerSecond = "";
                if (!headersSecond.isEmpty()) {
                    headerSecond = headersSecond.get(i);
                }
            ...
            for (String header : headers) {
                ...
                if ("Category".equals(field)) {
                    setHSSFValidation(sheet,
                            DealService.CURRENT_CATEGORIES.toArray(new String[DealService.CURRENT_CATEGORIES.size()]),
                            firstRow, endRow, indexCol, indexCol);
                } else {
                    String fieldType = DealService.FIELD_TYPE_RECORD.get(field);
                    if ("pick".equals(fieldType)) {
                        List<String> valueList = DealService.PICK_FIELD_RECORD.get(field);
